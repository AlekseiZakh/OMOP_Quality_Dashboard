...
class DashboardComponents:
    ...
    def render_metric_card(title: str, value: Any, delta: Optional[Any] = None, 
        ...
        try:
            ...
            logging.error(f"Error rendering metric card: {e}")
            st.error(f"Error rendering metric: {title}")
    ...
    def render_summary_cards(summary_data: Dict[str, Any], cols: int = 4):
        ...
        try:
            ...
            logging.error(f"Error rendering summary cards: {e}")
            st.error("Error rendering summary cards")
    ...
    def render_quality_score_gauge(score: float, title: str = "Quality Score"):
        ...
        try:
            ...
            logging.error(f"Error rendering quality score gauge: {e}")
            return None
    ...
    def render_status_distribution_pie(status_counts: Dict[str, int], title: str = "Check Status Distribution"):
        ...
        try:
            ...
            logging.error(f"Error rendering status distribution pie: {e}")
            return None
    ...
    def render_quality_trend_chart(trend_data: pd.DataFrame, x_col: str, y_col: str, 
        ...
        try:
            if trend_data.empty or x_col not in trend_data.columns or y_col not in trend_data.columns:
                st.warning("No valid trend data available for visualization")
                return None
            ...
            logging.error(f"Error rendering quality trend chart: {e}")
            return None
    ...
    def render_table_completeness_heatmap(completeness_data: List[Dict[str, Any]]):
        ...
        try:
            ...
            logging.error(f"Error rendering table completeness heatmap: {e}")
            return None
    ...
    def render_expandable_details(title: str, data: Any, key: str):
        ...
        try:
            ...
            logging.error(f"Error rendering expandable details: {e}")
            st.error(f"Error displaying details for {title}")
            ...
            with col3:
                ...
                if isinstance(data, pd.DataFrame):
                    try:
                        ...
                        logging.error(f"Error creating Excel download: {e}")
                        st.warning("Excel download temporarily unavailable")
        except Exception as e:
            logging.error(f"Error rendering download section: {e}")
            st.error("Error creating download buttons")
    ...
    def render_progress_bar(current: int, total: int, label: str = "Progress"):
        ...
        try:
            ...
            logging.error(f"Error rendering progress bar: {e}")
    ...
    def render_alert_box(alert_type: str, message: str, details: Optional[str] = None):
        ...
        try:
            ...
            logging.error(f"Error rendering alert box: {e}")
...
class QualityCheckRenderer:
    ...
    def render_check_results(results: Dict[str, Any], check_type: str):
        ...
        try:
            ...
            logging.error(f"Error rendering check results: {e}")
            st.error(f"Error displaying {check_type} results")
    ...
    def _render_individual_check(check_name: str, check_data: Dict[str, Any]):
        ...
        try:
            ...
            logging.error(f"Error rendering individual check: {e}")
            st.error(f"Error displaying check: {check_name}")
